...
export default function ReadingTab({
  ...
  async function loadHistory() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/study/readings?${params.toString()}`,
        { cache: "no-store" },
      );
  ...
  async function generate() {
    ...
    setGenerating(true);
    try {
      const res = await fetch("/api/study/readings/generate", {
        method: "POST",
  ...
  async function openReading(id: string) {
    try {
      const res = await fetch(
        `/api/study/readings/${encodeURIComponent(id)}`,
        { cache: "no-store" },
      );
      const data = (await res.json()) as
        | (Reading & { id: string })
  ...
  async function deleteReading(id: string) {
    ...
    await fetch(`/api/study/readings/${encodeURIComponent(id)}`, {
      method: "DELETE",
    }).catch(() => {});
  }
