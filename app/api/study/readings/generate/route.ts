...
function buildSystemPrompt(cefr: CefrLevel, category: ReadingCategory): string {
  ...
  return [
    "You are an experienced ESL reading-passage author.",
    "Generate a single short reading and return ONLY one JSON object — no commentary, no markdown fences.",
    "Schema:",
    '{ "title": string,',
    '  "body": string,',
    '  "summary": string,',
    ...
    `- Reading level must match CEFR ${cefr}: vocabulary, sentence length, and structure should feel natural for that level.`,
    `- Category: ${category}. ${tone}`,
    "- Title is short (max 8 words), title-case.",
    "- Summary is one sentence, 12-22 words, capturing the main idea.",
    '- Glossary contains 5 useful words pulled directly from the body. "meaning" is a short English paraphrase. "translation" is the Vietnamese translation. No transliterations.',
    "- 4 comprehension questions. Question ids r1..r4. Each has exactly 4 options. answer is a 0-based index. explanation is one short sentence.",
    "- Use natural English; avoid stilted textbook phrasing. Do not invent specific real-world facts that you cannot verify; prefer plausible, generic detail.",
  ].join(" ");
}
