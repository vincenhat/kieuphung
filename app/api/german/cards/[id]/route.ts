...
import { isValidArticle, isValidPos } from "@/lib/german";
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if ("pos" in body) {
    update.pos = isValidPos(body.pos) ? body.pos : null;
  }
  if ("article" in body) {
    update.article = isValidArticle(body.article) ? body.article : null;
  }
  if ("plural" in body) {
    update.plural = body.plural ? String(body.plural).slice(0, 120) : null;
  }
  if ("tags" in body) {
